...
function renderConfig({ name, theme, siteUrl, version, translation = '' }) {
  ...
  const lines = [
    ...
    '  scrollToTop: true,',
    '',
    '  // A button copying each block of code. It is the one thing that makes a',
    '  // content page load a script — a small file, shared by every page of the',
    '  // version — so it stays off until you ask for it.',
    '  copyCode: false,',
    '',
    '  // Icon collection a code block takes the mark of its language from, for',
    "  // instance 'simple-icons' once @iconify-json/simple-icons is installed.",
    '  // Left empty, the short name of the language is shown instead.',
    "  snippetIcons: '',",
    '',
