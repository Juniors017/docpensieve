...
<Admonition type="note" title="No questions?">
...
<Admonition type="tip" title="Why .mjs">
  The extension is what tells Node to read the file as a module. A `.js` file works only if a
  `package.json` nearby says so — and the one `npm init` writes says the opposite. The tool accepts
  both and refuses to guess when it finds two.
</Admonition>
