  ...
  <Column>
    <Card className="h-full" href="/components/">
      <CardBody className="text-sm text-slate-600 dark:text-slate-400">
        ...
        Twelve components usable without an import, none of them with JavaScript.
      </CardBody>
    </Card>
...
- **A menu that folds** — `foldedSidebar`, and the documentation menu folded
  above the content on a narrow screen.
- **A panel of links in the header** — a `headerLinks` entry with columns.
- **A menu component** — `Menu`, placed anywhere in a page, folded on mobile.
- **Admonitions** — six kinds shipped, and your own declared in the configuration.
- **A header held or not** — `stickyHeader`, to give its height back to the text.
- **Icons from a set** — `simple-icons:github`, inlined at the build like your
  own files.
- **A site in several languages** — a translation per version, served under its
  code, with the shell in that language.
