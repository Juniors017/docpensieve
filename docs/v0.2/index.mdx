...
- **Search** — a field in every header, a plain form that needs no script.
- **Sitemap, `robots.txt` and RSS** — for search engines and feed readers.
- **A menu written by hand** — `sidebar.json`, in each version's folder.
- **A fixed colour scheme, a logo per version** — `darkMode: 'dark'`,
  `versions[].logo`.
- **A light / dark switch** — on by default, remembered from page to page.
- **Lighter pages** — a minified stylesheet, sized and lazy-loaded images.
- **An API reference**, and two complete example projects.
