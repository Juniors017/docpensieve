  ...
  <Column>
    <Card className="h-full" href="/components/">
      <CardBody className="text-sm text-slate-600 dark:text-slate-400">
        ...
        Fifteen components usable without an import, and what they load, you choose.
      </CardBody>
    </Card>
...
- **A banner for the head of a page** — `Hero`, with its row of ways in.
- **A calendar** — the days your events fall on, navigable once loaded.
- **A button that copies a block of code** — `copyCode`, off by default: a
  page loads only what it uses.
- **A code block that names a file** — `Snippet` reads a real file of your
  project at generation, so an example cannot go stale.
- **init asks about languages** — one question, and the project comes out with
  its second language wired: the folder, the configuration field and a page to
  start from.

[Everything that is new](/whats-new/) ·
[Migrate from latest to beta](/guide/migrate-to-beta/)
...
`init` sets the project up through six questions. Enter keeps the default,
...
| A second language           | no                 | Yes, then a code: your pages get a folder of their own in that language — `docs/v1.0-fr/`, served under `/fr/`. A page with no twin in it does not exist in that language.             | `--translation fr`     |
...
| `docs/v1.0-fr/index.md`                                          | a second language — the home page, ready to translate                |
...
site, the light / dark switch and the button that copies a block of code:
about two kilobytes, on the pages that have code, and nowhere else.

That is the promise, and it is the one to hold: not that a page loads nothing,
but that it loads only what it uses.
