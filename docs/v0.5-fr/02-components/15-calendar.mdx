...
`compact` dessine la grille en petit et écrit les événements au-dessous. Il se
lit pareil à côté d'un paragraphe sur grand écran et sur un téléphone — où le
modèle complet devient une liste de jours, sept colonnes n'y laissant que
quelques millimètres à chacun.
...
| `compact`   | `boolean` | Le modèle réduit : grille serrée, et les événements listés.  |
...
Sur un écran étroit, les sept colonnes deviennent une liste de jours : une
semaine en colonnes sur un téléphone laisse quelques millimètres à chacun. Le
même choix est fait par [Columns](./columns/).
