...
Constantes, erreurs et slugs, partagés par tous les paquets.
...
**Renvoie** `string` — Lowercase slug, dash-separated.
...
**Renvoie** `string` — Slug with no leading or trailing slash.
...
**Renvoie** `string` — Output path, slash-separated.
...
**Renvoie** `string[]` — The offending segments, extension removed.
...
**Renvoie** `string` — URL starting and ending with `/`.
...
**Renvoie** `number` — The prefix number, or `Infinity` when absent (sorted last).
...
**Renvoie** `string` — For instance `12 pages` or `1 page`.
...
**Renvoie** `Promise<DocPensieveConfig>` — Normalised config.

**Lève** `ConfigError` — When no file, or two, are found, or when the file does not load or exports no object.
...
**Renvoie** `DocPensieveConfig` — Normalised config.

**Lève** `ConfigError` — When the config is structurally invalid.
...
**Renvoie** `Version` — The requested version.

**Lève** `ConfigError` — When the slug does not exist.
...
**Lève** `ConfigError` — For a path that names no page, a page listed twice, or an entry of no known kind.
...
**Renvoie** `Record<string, string>` — Full folder slug to title.
...
**Renvoie** `Map<string, Author>`

**Lève** `ConfigError` — When the shape is wrong, naming the offending entry.
...
**Lève** `ConfigError` — When a date cannot be read.
...
**Renvoie** `{ iso: string, label: string } \| null` — `null` when absent.

**Lève** `ConfigError` — When the value is not a date.
...
Les composants disponibles dans chaque page.
...
**Renvoie** `string \| undefined` — `undefined` when nothing is left, to avoid a `class=""` in the produced HTML.
...
**Lève** `DocPensieveError` — When the kind is unknown.
...
Documenté dans `packages/components/src/card.js`.
...
**Lève** `Error` — When the page list is missing, or `from` names nothing.
...
**Lève** `DocPensieveError` — Outside a `Columns`, or when the row mixes columns with and without a width.
...
**Lève** `DocPensieveError` — Outside a `Menu`, or without a title.
...
**Lève** `DocPensieveError` — Outside a `Menu`, or without a target.
...
**Lève** `DocPensieveError` — Without text, or with an unknown side.
...
**Lève** `DocPensieveError` — Outside a `Tree`, or without a label.
...
**Renvoie** `any` — Never: it is the calendar that draws its events, and this function exists to be recognised among a calendar children — and to say so when it stands anywhere else.

**Lève** `DocPensieveError` — Always, outside a calendar.
...
**Renvoie** `number` — 0 for Monday, 6 for Sunday.
...
**Renvoie** `(number \| null)[][]` — `null` where the cell belongs to no day.
...
**Renvoie** `{ year: number, month: number, day: number } \| null`
...
**Lève** `DocPensieveError` — With an alignment that does not exist.
...
**Lève** `DocPensieveError` — Without `src`, or when the file cannot be read.
...
**Lève** `DocPensieveError` — For a framework the configuration does not know, or when no theme was announced.
...
**Renvoie** `Promise<string>` — CSS, trimmed.

**Lève** `DocPensieveError` — When the stylesheet is missing.
...
**Renvoie** `string` — Absolute path, inside the source folder.

**Lève** `Error` — When the context is missing or the target escapes it.
...
**Renvoie** `string \| undefined` — The resolved target, or as is when external.
...
**Renvoie** `Record<string, Function>` — Table ready for @mdx-js/mdx.
...
**Renvoie** `string[]` — Names sorted alphabetically.

## `docpensieve`

Les commandes, appelables depuis un script comme depuis le terminal.
...
**Renvoie** `Promise<{ root: string, pages: number, faults: Fault[] }>`

**Lève** `DocPensieveError` — When the folder does not exist, or when something is left to fix — the exit code is then that of an expected error, which is enough to fail a continuous integration run.
...
**Renvoie** `Promise<{ pages: number, faults: Fault[] }>`
...
**Lève** `DocPensieveError` — When the folder to serve does not exist.
...
**Renvoie** `import('node:http').Server`
...
**Lève** `DocPensieveError` — When no port is free in the range.
...
**Renvoie** `string \| null` — Absolute path, or `null` when the target escapes `root`.
